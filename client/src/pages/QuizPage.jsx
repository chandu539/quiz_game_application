...
function QuizPage() {
    ...
    useEffect(() => {
        const fetchQuestions = async () => {
            try {
                const response = await axios.get('https://quiz-game-application-chi.vercel.app/api/questions');
                if (response.data.length === 0) {
                    await axios.post('https://quiz-game-application-chi.vercel.app/api/questions/store', { questions: predefinedQuestions });
                    toast.success("Questions stored successfully!");
                    const updatedResponse = await axios.get('https://quiz-game-application-chi.vercel.app/api/questions');
